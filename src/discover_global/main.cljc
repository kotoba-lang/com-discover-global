(ns discover_global.main
  "Kotodama WASM entrypoint for the discover_global clean-room actor (L4) — Clojure port.
  ...
  facts (`discover_global.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "discover_global")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"discover_global.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "discover_global-compat" :tier tier :entities entities} 200])
...
(defn start! [] :discover_global-compat/ready)
